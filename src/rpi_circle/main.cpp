...
void renderFatalScreen(lv_display_t *display, const char *title, const char *detail) {
    if (!display) {
        return;
    }
    ...
    lv_label_set_text(detail_label, detail && detail[0] != '\0' ? detail : "unknown");
...
class FluidTouchCircleKernel : public CStdlibAppStdio {
  public:
    ...
    bool Initialize() override {
        signalStage(mActLED, BootStage::StdlibInit);
        if (!CStdlibAppStdio::Initialize()) {
            signalStage(mActLED, BootStage::InitFailed);
            return false;
        }

        signalStage(mActLED, BootStage::TouchInit);
        touch_screen_.Initialize();

        signalStage(mActLED, BootStage::LvglInit);
        if (!gui_.Initialize()) {
            mLogger.Write(GetKernelName(), LogError, "LVGL initialization failed");
            return false;
        }

        renderPreAppSmokeScreen(lv_display_get_default());
        signalStage(mActLED, BootStage::AppInit);
        try {
            ...
            renderFatalScreen(lv_display_get_default(), "Init exception", "unknown exception");
            mLogger.Write(GetKernelName(), LogPanic, "Init exception: unknown");
            return false;
        }
        mLogger.Write(GetKernelName(), LogNotice, "FluidTouch shared UI initialized");
        return true;
    }

    void Cleanup() override {
        CStdlibAppStdio::Cleanup();
    }
    ...
    TShutdownMode Run() override {
        ...
        while (!EnvPlatform::isExitRequested()) {
            try {
                ...
                renderFatalScreen(lv_display_get_default(), "Runtime exception", ex.what());
                mLogger.Write(GetKernelName(), LogPanic, "Runtime exception: %s", ex.what());
                for (;;) {
                    cpu_throttle_.Update();
                }
            } catch (...) {
                renderFatalScreen(lv_display_get_default(), "Runtime exception", "unknown exception");
